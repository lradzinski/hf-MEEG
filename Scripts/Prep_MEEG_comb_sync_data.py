...
plots_output_folder = os.path.join(plots_output_folder, date, subject_name)
...
sigma_rms_st = np.sqrt(np.mean(MEG_sigma_band_whole_trials[0][meg_srate*15//1000:meg_srate*30//1000]**2, axis=0))
noise_rms_st = np.sqrt(np.mean(MEG_sigma_band_whole_trials[0][meg_srate*50//1000:meg_srate*200//1000]**2, axis=0))
snnr_st = np.mean(sigma_rms_st/noise_rms_st)

sigma_rms_er = np.sqrt(np.mean(MEG_sigma_band_trials_averaged[0][meg_srate*15//1000:meg_srate*30//1000]**2))
noise_rms_er = np.sqrt(np.mean(MEG_sigma_band_trials_averaged[0][meg_srate*50//1000:meg_srate*200//1000]**2))
...
for i in range(3):
    for j in range(3):
        ...
        axs[i][j].plot(data_x, data_y, linewidth=1, label='EEG %s' % channames_aligned[cnt])
...
for i in range(3):
    for j in range(3):
        ...
        axs[i][j].set_ylim((-ylim, ylim))
        axs[i][j].set_ylabel(eeg_unit)
        axs[i][j].legend(prop={'family': 'DejaVu Sans Mono'}, title='EEG ch         snnr')
        if(i==2):
            axs[i][j].set_xlabel('t [ms]')
        ...
        if(cnt >= 8):
            ...
            axs[2][2].plot([], [], linewidth=1, label='%s st: %5.2f' % ('ch0', snnr_st), c='tab:blue')
            axs[2][2].plot(data_x, data_y, linewidth=1, label='%s er: %5.2f' % ('ch0', snnr_er))

            ylim = (1+np.max(np.abs(MEG_sigma_band_trials_averaged)[:, :meg_srate//10,])//5)*5
            axs[2][2].set_ylim((-ylim, ylim))

            axs[2][2].set_ylabel(meg_unit)
            axs[2][2].legend(prop={'family': 'DejaVu Sans Mono'}, title='MEG ch         snnr')
            axs[2][2].set_xlabel('t [ms]')
